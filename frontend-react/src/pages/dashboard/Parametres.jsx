...
const T = {
  FR: {
    title:'Paramètres', sub:'Configuration du système',
  ...
  EN: {
    title:'Settings', sub:'System configuration',
...
export default function Parametres({ theme, lang }) {
  ...
  async function saveCode() {
    if (nouveauCode.trim().length < 6) { showToast(t.errorMin, 'error'); return }
    setSaving(true)
    try {
      const token = localStorage.getItem('sdi_token')
      const res   = await fetch(`${API}/api/auth/invite-code`, {
    ...
    <>
      <div className="admin-top">
        ...
        <div className="admin-top-r">
          <button className="btn btn-secondary btn-sm" onClick={loadCode} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <RefreshCw size={13} />
            {t.refresh}
          </button>
        </div>
      ...
      {toast.msg && (
        <div style={{
          padding: '12px 18px', borderRadius: 12, marginBottom: 16, fontSize: 13, fontWeight: 500,
      ...
      <div className="panel" style={{ background: cardBg, borderColor: border, maxWidth: 600 }}>
        ...
        {/* Code actuel — oeil seulement, pas de bouton copier */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 12, color: ink3, marginBottom: 6, fontWeight: 500 }}>{t.codeActuel}</div>
          <div style={{ position: 'relative' }}>
            <input
              type={showCode ? 'text' : 'password'}
              value={loading ? '...' : codeActuel}
              readOnly
              style={{ ...inputStyle, background: isDark?'rgba(139,92,246,0.06)':'rgba(139,92,246,0.04)' }}
            />
            {/* Seulement l'oeil — bouton copier supprimé */}
            <button
              onClick={() => setShowCode(!showCode)}
              style={{
                position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)',
                background: 'none', border: 'none', color: showCode ? '#A78BFA' : ink4,
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 12, color: ink3, marginBottom: 6, fontWeight: 500 }}>{t.nouveauCode}</div>
          <div style={{ position: 'relative' }}>
            <input
            ...
            <button
              ...
              style={{
                position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)',
                background: 'none', border: 'none', color: showNew ? '#A78BFA' : ink4,
                cursor: 'pointer', display: 'flex', alignItems: 'center', padding: 4,
                transition: 'color 0.15s',
              }}
            >
        ...
        {/* Info */}
        <div style={{ padding: '10px 14px', borderRadius: 10, marginBottom: 16, background: infoBg, border: `1px solid ${infoBdr}`, display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 12, color: isDark?'#93C5FD':'#3773bd' }}>
        ...
        <button
          ...
          style={{
            ...
            border: 'none',
            color: nouveauCode.trim() ? 'white' : ink4,
